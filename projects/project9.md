...
- Erstelle eine Karte Europas, in welcher Du den Wert von `netNEE` für jede Station farblich kodierst (mit horizontalem Colorbar als Legende). Stelle zur räumlichen Verortung das Shapefile mit den europäischen Ländern mit dar. Inwiefern sticht die Station "SE-Nor" hervor? Versuche zu recherchierenn, was ist dort passiert ist. Zeige eine Zeitreihe von `NEE` für diese Station.
- Schreibe eine Funktion, welche für eine beliebige Station eine zweispaltige Abbildung erstellt:
   - linke Spalte: mittlere Tagesgänge (stündliche Mittelwerte) von LE, NEEip sowie der kurzwelligen Einstrahlung für Januar und Juli
